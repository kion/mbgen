...
Post body.`
	...
	// verify tag display name is stored as-is
	if len(post.Tags) != 1 || post.Tags[0] != testTagMultiWordMetadata {
		t.Errorf("Expected tag %q, got %v", testTagMultiWordMetadata, post.Tags)
	}

	// verify the normalized URI appears in the rendered body (via post-tags.html template)
	// The URI is generated by the template using normalizeTagURI, so we verify
	// that normalizeTagURI produces the expected slug
	if normalizeTagURI(post.Tags[0]) != testTagMultiWordNormalized {
		t.Errorf("normalizeTagURI(%q) = %q, want %q", post.Tags[0], normalizeTagURI(post.Tags[0]), testTagMultiWordNormalized)
	}
}
...
Post body with #HashTagAlpha and #HashTagBeta inline.`
	...
	// only the explicitly listed YAML metadata tag should be present
